...
from coscientist.model_gateway import gateway
...
class CoscientistFramework:
    ...
    async def start(self, n_hypotheses: int = None) -> None:
        ...
        k_bracket = min(settings.TOURNAMENT_BRACKET_K, 2 ** math.floor(math.log2(n_hypotheses)))
    ...
    async def run(self) -> tuple[str, str]:
        ...
        """
        # Start off with initial hypotheses
        if not self.state_manager.is_started:
            _ = await self.start(n_hypotheses=settings.INITIAL_HYPOTHESES)
        ...
        while not self.state_manager.is_finished:
            ...
            if current_action == "finish" and settings.REQUIRE_HUMAN_APPROVAL:
                ...
                if user_input.strip().lower() != 'y':
                    print("=> Human rejected finish action. Forcing 'run_tournament' instead.")
                    current_action = "run_tournament"
                    final_supervisor_state["action"] = current_action
                    final_supervisor_state["decision_reasoning"] = "[HITL OVERRIDE] " + final_supervisor_state["decision_reasoning"]
            # --------------------------------------
