...
from coscientist.agents.literature_review_agent import LiteratureReviewState
...
from coscientist.models.db_models import Goal, Hypothesis, Review
...
class CoscientistState:
    ...
    def __init__(self, goal: str):
        ...
        # Initialize goal in SQLite
        with SessionLocal() as db:
            db_goal = db.query(Goal).filter(Goal.id == goal_hash).first()
            if not db_goal:
                db_goal = Goal(id=goal_hash, description=goal)
                db.add(db_goal)
                db.commit()
    ...
    def load_latest(
        ...
        for checkpoint in checkpoints:
            try:
                state = cls.load(checkpoint)
                logger.info(f"Successfully loaded checkpoint: {checkpoint}")
                return state
            except Exception as e:
                logger.warning(f"Failed to load checkpoint {checkpoint}: {e}. Trying older checkpoint if available.")
...
class CoscientistStateManager:
    ...
    @property
    def tournament(self) -> "EloTournament":
        """The Elo tournament state."""
        return self._state.tournament

    @property
    def proximity_graph(self) -> "ProximityGraph":
        """The semantic proximity graph."""
        return self._state.proximity_graph
    ...
    def add_generated_hypothesis(
        ...
        with SessionLocal() as db:
            db_hypo = Hypothesis(
                uid=hypothesis.uid,
                goal_id=CoscientistState._hash_goal(self.goal),
    ...
    def add_reviewed_hypothesis(self, reviewed_hypothesis: ReviewedHypothesis) -> None:
        ...
        semantic_memory.add_hypothesis(
            uid=reviewed_hypothesis.uid,
            hypothesis_text=reviewed_hypothesis.hypothesis,
            elo=reviewed_hypothesis.elo
        )
        semantic_memory.add_feedback(
            uid=reviewed_hypothesis.uid,
            feedback_text=reviewed_hypothesis.verification_result
        )
        
        # Save to SQLite
        with SessionLocal() as db:
            db_hypo = db.query(Hypothesis).filter(Hypothesis.uid == reviewed_hypothesis.uid).first()
            if db_hypo:
                db_hypo.stage = "reviewed"
            else:
                db_hypo = Hypothesis(
                    uid=reviewed_hypothesis.uid,
                    goal_id=CoscientistState._hash_goal(self.goal),
                    content=reviewed_hypothesis.hypothesis,
                    stage="reviewed"
                )
                db.add(db_hypo)
            
            db_rev = Review(
                hypothesis_uid=reviewed_hypothesis.uid,
                verification_result=reviewed_hypothesis.verification_result
            )
            db.add(db_rev)
            db.commit()
    ...
    def add_evolved_hypothesis(
        ...
        with SessionLocal() as db:
            db_hypo = Hypothesis(
                uid=evolved_hypothesis.uid,
                goal_id=CoscientistState._hash_goal(self.goal),
                content=evolved_hypothesis.hypothesis,
                stage="evolved"
            )
            db.merge(db_hypo)
            db.commit()
    ...
    @_maybe_save(n=1)
    async def run_tournament_async(self, llm: BaseChatModel, k_bracket: int = 16) -> None:
        """
        Run the tournament asynchronously.
        """
        assert self._state.tournament is not None, "Tournament is not initialized"
        if hasattr(self._state.tournament, "run_tournament_async"):
            await self._state.tournament.run_tournament_async(llm=llm, k_bracket=k_bracket)
        else:
            # Fallback for now until Phase 4
            self._state.tournament.run_tournament(llm=llm, k_bracket=k_bracket)
