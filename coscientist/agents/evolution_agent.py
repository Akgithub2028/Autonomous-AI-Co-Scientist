...
async def _evolve_from_feedback_node(
    ...
    prompt = load_prompt(
        ...
        review=truncate_text(state["parent_hypothesis"].verification_result, 500),
        meta_review=truncate_text(state["meta_review"], 500),
    )
...
async def _out_of_the_box_node(
...
def _build_evolve_from_feedback_agent(llm: BaseChatModel) -> StateGraph:
    ...
    graph.add_node(
        "evolve_from_feedback",
        functools.partial(_evolve_from_feedback_node, llm=llm),
    )
    graph.add_edge("evolve_from_feedback", END)

    graph.set_entry_point("evolve_from_feedback")
...
def _build_out_of_the_box_agent(llm: BaseChatModel) -> StateGraph:
    ...
    graph.add_node(
        "out_of_the_box",
        functools.partial(_out_of_the_box_node, llm=llm),
    )

    graph.add_edge("out_of_the_box", END)

    graph.set_entry_point("out_of_the_box")
    return graph.compile()
