...
def build_supervisor_agent(llm: BaseChatModel) -> StateGraph:
    ...
    graph.add_node(
        "supervisor_decision",
        functools.partial(_supervisor_decision_node, llm=llm),
    )
...
def _parse_supervisor_response(response: str) -> tuple[str, str]:
    ...
    # Fallback to prevent crashes if LLM output format is unexpected
    if not action:
        action = "run_tournament"
        decision_reasoning = "Fallback: Could not parse DECISION. Defaulting to tournament."
...
async def _supervisor_decision_node(
    ...
    # Another safety check for action name
    valid_actions = [
