...
from coscientist.services.common import load_prompt, parse_hypothesis_markdown, truncate_text
...
class CollaborativeConfig:
    ...
    max_turns: int = settings.MAX_DEBATE_TURNS
...
async def _independent_generation_node(
    ...
    """
    from coscientist.retrieval.retrieval_crag import corrective_rag_search
    from coscientist.memory.memory_store import semantic_memory
    
    # Handle meta_review field with fallback
    meta_review = state.get("meta_review", "Not Available")

    # Perform a CRAG search for this specific field
    query = f"Novel {field} mechanisms and hypotheses for: {state['goal']}"
    crag_results = await corrective_rag_search(query, max_iterations=1, context=state["goal"])

    # Retrieve past relevant hypotheses from long-term memory
    past_memories = semantic_memory.retrieve_relevant_memories(state['goal'], k=3, filter_type="hypothesis")
    past_memory_text = "\n".join([doc.page_content for doc in past_memories]) if past_memories else "No previous hypotheses available."

    extended_literature = f"{truncate_text(state['literature_review'], 800)}\n\nField-Specific Research ({field}):\n{truncate_text(crag_results, 800)}\n\nPast Hypotheses in Memory:\n{truncate_text(past_memory_text, 500)}"

    prompt = load_prompt(
        "independent_generation",
        goal=state["goal"],
        field=field,
        literature_review=extended_literature,
        meta_review=truncate_text(meta_review, 500),
...
def _build_independent_generation_agent(
    ...
    graph.add_node(
        "generator",
        functools.partial(_independent_generation_node, field=field, reasoning_type=reasoning_type, llm=llm),
    )
...
def _build_collaborative_generation_agent(
    ...
    for agent_name in agent_names:
        # Make sure to truncate long context strings here too
        # But for state keys we can't easily truncate inside create_agent_node_fn automatically
        # Let's just create a wrapper function or modify the state first
        
        # We can't modify the state easily inside the loop without creating a custom node
        # But we can rely on the prompt being loaded with these keys from state
        # The common.py update added truncate_text, but we can't inject it here without rewriting create_agent_node_fn
        # Let's just pass the keys
...
def _termination_fn(msg: str) -> bool:
    ...
    for section in sections:
        ...
        if "paradigm" in title:
            has_paradigm = True
        elif "hypothesis" in title:
            has_hypothesis = True
    ...
    return has_paradigm and has_hypothesis and has_predictions and has_assumptions
